...
$num_results4 = mysql_num_rows($result4);
   ...
   for ($i2=0; $i2 < $num_results4; $i2++)
  {
	...
	$entriesin= file_get_contents("http://localhost/ws/weather_get.php?station=".$row['station_no']."&updates=".$updates, "r");
	echo $entriesin;
 ...
 else
 {
   ...
   $query = "SELECT *,DATE_FORMAT(ttl,'%h:%i %p %d %b %Y') as disp_ttl FROM station WHERE station_no=".$station;
  $result = mysql_query($query) or die('<p>Error executing query</p>');
$num_results = mysql_num_rows($result);
...
$num_results4 = mysql_num_rows($result4);
   ...
   for ($i2=0; $i2 < $num_results4; $i2++)
  {
	...
	$entriesin= file_get_contents("weather_get.php?station=".$row['station_no']."&updates=".$updates, "r");
	echo $entriesin;
